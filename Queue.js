...
// output:  ถ้า list มีข้อมูลจะ return ข้อมูล , ถ้า list ว่างจะ return null
  dequeue() {
    ...
    this.size--;
    this.tail--; // เวลา dequeue ออกแล้ว tail ไม่ยอมลด เลยต้องเพิ่ม this.tail
    this.head++;
    if(this.head == this.max) {
      this.head=0; // ตอนแรกเป็น this.head==0 มันเป็นการเช็คว่า เท่ากันไหม แต่ เราต้องการให้ this.head ให้เท่ากับ 0 เลยต้องเอา = ออก 1 อันเลยเป็น this.head = 0
    }
    return x;
  }
}
